...
class StripeHandlerView(APIView):
    ...
    def post(self,request,*args,**kwargs):
        try:
            ...
            if data['product_data']['product_hash'] not in sent_user_feedback:
                sent_user_feedback[data['product_data']['product_hash']]=False
            ...
            if data["type"] == "PAID":
                res_data=requests.post("http://localhost:8080/wallet/transfer",json={
                        "usdcxAmount":float(amount)*prod_obj.quantity,
                ...
                if sent_user_feedback[data['product_data']['product_hash']]==False:
                    reply_whatsapp_message(msg=msg,to=wallet_obj[0].number_id)
                    prod_obj.is_paid=True
                    prod_obj.save()
                    sent_user_feedback[data['product_data']['product_hash']]=True
                    print("PAID")
...
class OnChainPaymentView(APIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        return Response({ "Info":"Merchant Notified"})
